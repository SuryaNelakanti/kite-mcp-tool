...
class WebpageScraperTool(BaseTool):
    ...
    def _extract_metadata(self, soup: BeautifulSoup, doc: Document, url: str) -> WebpageMetadata:
        ...
        if description_tag:
            metadata["description"] = description_tag.text

        site_name_tag = soup.find("meta", attrs={"property": "og:site_name"})
        if site_name_tag:
            metadata["site_name"] = site_name_tag.text
    ...
    def run(self, params: WebpageScraperToolInputSchema) -> WebpageScraperToolOutputSchema:
        ...
        try:
            ...
            minimal_metadata = WebpageMetadata(author=None, description=None, site_name=str(params.url), title="Error retrieving page", domain=domain)
    ...
    try:
        result = scraper.run(
            WebpageScraperToolInputSchema(
                url=HttpUrl("https://github.com/BrainBlend-AI/atomic-agents"),
                include_links=True,
            )
        )
